...
// No table modules needed - using simple HTML insertion
...
const ComposeEmail: React.FC<ComposeEmailProps> = ({
  ...
  useEffect(() => {
    if (replyTo) {
      // Reply: Set sender as recipient, add "Re: " prefix to subject
      setTo([replyTo.sender.email]);
      setSubject(replyTo.subject.startsWith('Re: ') ? replyTo.subject : `Re: ${replyTo.subject}`);
      setBody(`<br><br>--- Original Message ---<br><strong>From:</strong> ${replyTo.sender.firstName} ${replyTo.sender.lastName} &lt;${replyTo.sender.email}&gt;<br><strong>Date:</strong> ${new Date(replyTo.sentAt).toLocaleString()}<br><strong>Subject:</strong> ${replyTo.subject}<br><br>${replyTo.body}`);
    } else if (forwardEmail) {
      // Forward: Clear recipients, add "Fwd: " prefix to subject, include attachments
      setTo([]);
      setSubject(forwardEmail.subject.startsWith('Fwd: ') ? forwardEmail.subject : `Fwd: ${forwardEmail.subject}`);
      setBody(`<br><br>--- Forwarded Message ---<br><strong>From:</strong> ${forwardEmail.sender.firstName} ${forwardEmail.sender.lastName} &lt;${forwardEmail.sender.email}&gt;<br><strong>Date:</strong> ${new Date(forwardEmail.sentAt).toLocaleString()}<br><strong>Subject:</strong> ${forwardEmail.subject}<br><br>${forwardEmail.body}`);
  ...
  const handleToKeyPress = (e: React.KeyboardEvent) => {
    if (e.key === 'Enter' || e.key === ',') {
      ...
      removeToRecipient(to[to.length - 1]);
    }
  };

  // Handle CC input key press
  const handleCcKeyPress = (e: React.KeyboardEvent) => {
    if (e.key === 'Enter' || e.key === ',') {
      ...
      removeCcRecipient(cc[cc.length - 1]);
    }
  };

  // Add distribution list members to recipients
  const addDistributionList = async (listId: string, field: 'to' | 'cc') => {
    try {
      const list = await DistributionListService.getListById(listId);
      if (list) {
        const emails = list.members.map(member => member.email);
        if (field === 'to') {
          const newEmails = emails.filter(email => !to.includes(email) && !cc.includes(email));
          setTo([...to, ...newEmails]);
        } else {
          const newEmails = emails.filter(email => !cc.includes(email) && !to.includes(email));
          setCc([...cc, ...newEmails]);
        }
      }
      setShowDistributionLists(false);
    } catch (error) {
      console.error('Error adding distribution list:', error);
      setShowDistributionLists(false);
    }
  };
  ...
  // TinyMCE handles paste events internally through the editor configuration

  // Add paste event listener to TinyMCE editor
  useEffect(() => {
    if (editorRef.current) {
      // TinyMCE handles paste events internally through the editor configuration
      console.log('TinyMCE editor ready for paste handling');
    }
  }, []);
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg shadow-2xl w-full max-w-3xl max-h-[80vh] flex flex-col">
        ...
        <div className="flex-1 flex flex-col">
          ...
          <div className="sticky top-0 z-30 bg-white space-y-2 p-3 border-b border-gray-200 flex-shrink-0">
            {/* TO Field */}
            <div className="relative">
              <div className="flex items-center space-x-3">
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <div className="relative">
                    {/* Lists button - Hidden for BDE users */}
                    {!isBDEUser && (
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
                    ...
                    {/* Distribution Lists Dropdown - Hidden for BDE users */}
                    {!isBDEUser && showDistributionLists && (
                  ...
                  <div className="flex-1 relative">
                    <div className="flex flex-wrap items-center gap-2 p-2 border border-gray-300 rounded-lg focus-within:ring-2 focus-within:ring-primary-500 focus-within:border-primary-500">
                      ...
                      <input
                        ...
                        className="flex-1 min-w-0 outline-none text-sm"
                        placeholder={cc.length === 0 ? "Enter CC addresses..." : ""}
                      />
                    </div>
          ...
          <div className="flex-1 border-t border-gray-200 overflow-auto">
            <Editor
              onInit={(evt, editor) => {
        ...
        <div className="flex items-center justify-between p-4 border-t border-gray-200 bg-white">
          <div className="flex items-center space-x-3">
            ...
            <button
              ...
              title={
                loadingAttachmentPermission
                  ? 'Checking attachment permissions...'
                  : !canSendAttachments
        ...
        {/* Mail Merge Modal */}
        {showMailMerge && (
          <MailMerge onClose={() => setShowMailMerge(false)} />
        )}
      </div>
    </div>
  );
